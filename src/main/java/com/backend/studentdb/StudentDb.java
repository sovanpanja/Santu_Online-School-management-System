...
public class StudentDb {
	...
	public int insertData(String f1, String f2, String f3, String f4, String f5, String f6, String f7, String f8,
		...
		connect();
		ps = con.prepareStatement("insert into student(mobile,fname,lname,dept,dob,gender,gname,addr,email,pwd,session,category) values(?,?,?,?,?,?,?,?,?,?,?,?)");
		ps.setString(1, f1);
		ps.setString(2, f2);
		ps.setString(3, f3);
		ps.setString(4, f4);
	...
	public Vector<String> getName (String f1) throws Exception {
		connect();
		Vector<String> v = new Vector<String>();
		ps = con.prepareStatement("Select fName,lName from student where mobile=?");
		ps.setString(1, f1);
		rs = ps.executeQuery();
		while (rs.next()) {
	...
	public boolean checkLogin(String f1, String f2) throws Exception {
		connect();
		boolean b = false;
		ps = con.prepareStatement("Select * from student where mobile=? and pwd=?");
		ps.setString(1, f1);
		ps.setString(2, f2);
		rs = ps.executeQuery();
		while (rs.next()) {
	...
	public String getOldPassword(String f1) throws Exception {
		String pass = null;
		connect();
		ps = con.prepareStatement("select pwd from student where mobile=?");
		ps.setString(1, f1);
		rs = ps.executeQuery();
		while (rs.next()) {
			pass = rs.getString(1);
	...
	{
	connect();
	ps = con.prepareStatement("update student set pwd=? where pwd=?");
	ps.setString(1,f2);
	ps.setString(2,f1);
	int i = ps.executeUpdate();
	con.close();
